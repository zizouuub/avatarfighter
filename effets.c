...
void mettreAJourEffets(Combattant *c, TechniqueSpeciale *tech) {
    ...
    if (tech != NULL && tech->effet != AUCUN) {
        appliquerEffetElementaire(c, *tech);
    }
...
float multiplicateur(Element attaquant, Element defenseur) {
    if (attaquant == FEU && defenseur == TERRE) return 1.5;
    if (attaquant == EAU && defenseur == FEU) return 1.5;
    if (attaquant == TERRE && defenseur == AIR) return 1.5;
    if (attaquant == AIR && defenseur == TERRE) return 1.5;
    return 1.0;
}
